...
> **Aggiornato 2026-05-10 (deep audit closure follow-up + CLAUDE.md refresh): CLAUDE.md "Current versions" / "Project status" / "Pending work" sezioni stale 13 giorni REFRESHED ([commit `a659979`](https://github.com/istefox/obsidian-mcp-connector/commit/a659979): main 0.3.10→0.3.12, feat/http-embedded 0.4.0-alpha.4→0.4.5, tag protection scope estesa a `0.x.x` glob, Project status rewrite con tag stacks + 26 tools + [Unreleased] batch staged, Pending work re-prioritized con store monitor first + 0.4.6 cut ready + marcoaperez passive + jacksteamdev DM gated). Delta check commenti 24h: ZERO novità rilevanti — tutti i thread tracciati invariati (fork #88 closed mio, #54 closed mio, #67/#68/#77 quiescenti dal 2026-05-04; upstream cluster invariato dal 2026-05-08 16:23Z; **store #11919 untouched 3+ giorni, 0 reviews/0 review_requests/0 human assignees, week 4/8 silenzio normale**); folotp attivo sul suo `folotp/organon-plugin` personale (non verso fork); marcoaperez day 5/14 silenzio. **HEAD `feat/http-embedded` = `a659979`** (11 commit avanti vs #87 baseline pre-2026-05-08).**
>
...
- **`README.md` "The other 19 tools work without it [LRA]"** stale: tool count post-0.4.5 è 26 totali (25 LRA-free + 1 LRA-using `search_vault`, ora dynamic-port via #90). Drift cosmetico minore, edit mirato 1 riga.
